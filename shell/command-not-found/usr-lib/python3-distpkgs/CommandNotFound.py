...
class CommandNotFound(object):
    ...
    def print_spelling_suggestions(self, word, mispell_packages, mispell_snaps, max_alt=15):
        ...
        if len(mispell_packages)+len(mispell_snaps) > max_alt:
            ...
            print(_("Command '%s' is not in the Artemis Shell, but there are %s packages with similar commands.") % (word, len(mispell_packages)), file=self.output_fd)
            print("", file=self.output_fd)
            self.output_fd.flush()
            return
        elif len(mispell_packages)+len(mispell_snaps) > 0:
            print("", file=self.output_fd)
            print(_("Command '%s' is not in the Artemis Shell, did you mean this command instead?") % word, file=self.output_fd)
            print("", file=self.output_fd)
            ...
            for (command, package, comp, ver) in mispell_packages:
                if ver:
                    ...
                    ver = ""
                print(_("  command '%s' from apt:%s%s") % (command, package, ver), file=self.output_fd)
        print("", file=self.output_fd)
        if len(mispell_snaps) > 0:
            print(_("See 'snap info <snapname>' for additional versions of this package."), file=self.output_fd)
    ...
    def _print_exact_header(self, command):
        print(file=self.output_fd)
        print(_("Command '%(command)s' is not in Artemis Shell, but can be installed with:") % {
    ...
    def advice_single_snap_package(self, command, packages, snaps):
        ...
        if self.euid == 0:
            ...
            print("snap install %s" % snap[0], file=self.output_fd)
            print(_("Please ask your Artemis Shell administrator."))
        print("", file=self.output_fd)                    
        self.output_fd.flush()
    ...
    def advice_single_deb_package(self, command, packages, snaps):
        ...
        if self.euid == 0:
            ...
            print("apt install %s" % packages[0][0], file=self.output_fd)
            print(_("Please ask your administrator for Artemis Shell."))
            if not packages[0][2] in self.sources_list:
                print(_("You will have to enable the component '%s'") % packages[0][2], file=self.output_fd)
        print("", file=self.output_fd)                    
        self.output_fd.flush()
    ...
    def advice_multi_deb_package(self, command, packages, snaps):
        ...
        for i, package in enumerate(packages):
            ...
            if package[2] in self.sources_list:
                ...
                print("%sapt install %-*s%s" % (self.sudo(), pad, package[0], ver) + " (" + _("You will have to enable the component '%s'") % package[2] + ")", file=self.output_fd)
        if self.euid != 0 and not self.user_can_sudo:
            print("", file=self.output_fd)
            print(_("Ask your Artemis Shell administrator to install one of them."), file=self.output_fd)
        print("", file=self.output_fd)
        self.output_fd.flush()
    ...
    def advice_multi_snap_packages(self, command, packages, snaps):
        ...
        print("", file=self.output_fd)
        print(_("See 'snap info <snapname>' for additional versions of the packages."), file=self.output_fd)
        print("", file=self.output_fd)
        self.output_fd.flush()
    ...
    def advice_multi_mixed_packages(self, command, packages, snaps):
        ...
        print("", file=self.output_fd)
        if len(snaps) == 1:
            print(_("See 'snap info %s' for additional versions of the packages.") % snaps[0][0], file=self.output_fd)
        else:
            print(_("See 'snap info <snapname>' for additional versions of the packages."), file=self.output_fd)
        print("", file=self.output_fd)
        self.output_fd.flush()
    ...
    def advise(self, command, ignore_installed=False):
        ...
        if prefixes and not ignore_installed:
            ...
            if len(missing) > 0:
                print(_("'%s' could not be detected by Artemis Shell as it is not in the PATH environment variable.") % ":".join(missing), file=self.output_fd)
                if "sbin" in ":".join(missing):
                    print(_("This is probably because you do not have an account with administrator privileges."), file=self.output_fd)
            return False
        ...
        if not (os.path.exists("/usr/bin/apt") or
            ...
            print("Artemis Shell could not detect apt or aptitude, so it could not find similar packages. Apt can be re-downloaded at packages.ubuntu.com.")
            return False
        ...
        if command == "python":
            print("")
            print("Python is not in the Artemis Shell, and should not be used. Use Python3 instead with python3.")
            print("")
            return True 
